...
class Users_model extends CI_Model{	
    ...
    function save($data){
        $this->db->insert('user_learning', $data);
    }
    
    function delete($username){
        $this->db->where('username', $username)
                 ->delete('user_learning');
    }
    
    function update($data, $username){
        $this->db->where('username', $username)
                 ->update('user_learning', $data);
    }
	...
	function get_login_info($username){
		$this->db->where('username',$username);
		$this->db->limit(1);
		$query = $this->db->get('user_learning');
    ...
    function change_password($username, $password){
        $this->db->where('username', $username);
        $this->db->update('user_learning', array('password' => sha1($password)));
	}
